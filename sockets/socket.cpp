...
Socket::Socket(int domain)
{
    if (domain != AF_UNIX && domain != AF_INET && domain != AF_INET6) {
        throw std::runtime_error("Domain has to be one of these three options: AF_UNIX, AF_INET, AF_INET6.");
    }
...
Socket::bind(Address& address)
{
    if (address.domain() != s_domain)
        throw std::runtime_error("You are trying to bind to an address of wrong domain.");
...
{
    if (address.domain() != s_domain)
        throw std::runtime_error("You are trying to connect to an address of wrong domain.");
...
Socket::accept()
{
    ...
    std::unique_ptr<Address> addr = std::make_unique<IPv4Address>(reinterpret_cast<sockaddr*>(&address));
...
void
Socket::set_address(std::unique_ptr<Address> address)
{
    s_addr = std::move(address);
}

const Address& 
Socket::get_address() const {
    if (!s_addr)
...
void
Socket::read(void* buffer, int buffer_size)
{
...
void
Socket::send(const char* buffer, int buffer_size)
{
